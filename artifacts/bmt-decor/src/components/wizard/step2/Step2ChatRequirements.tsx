...
import { calcConstruction, parseSetbackInput, formatBudgetLabel, UNIT_COST_MILLION_PER_M2 } from "./calculation";
import type { ChatMessage, ChatStage, ExtractedFacts, TechnicalSummary, Step2ChatOutput, SetbackMode } from "./types";
  ...
  return (
    <div className="flex items-end gap-2 px-1">
      <div
        ...
        style={{ background: "rgba(255,123,0,0.18)", color: "#ff9500", border: "1px solid rgba(255,123,0,0.25)" }}
      >
        AI
      </div>
      <div
...
function TechnicalSummaryCard({ summary }: { summary: TechnicalSummary }) {
  const rows: { label: string; value: string; highlight?: boolean }[] = [
    { label: "Diện tích đất", value: `${summary.landWidth}×${summary.landLength}m = ${(summary.landWidth * summary.landLength).toFixed(0)}m²` },
    { label: "Khoảng chừa sân", value: summary.setback === 0 ? "Không chừa (xây hết đất)" : `${summary.setback}m` },
    ...
    { label: `Thân (${summary.floors} tầng × 100%)`, value: `${summary.bodyArea.toFixed(1)}m²` },
  ...
  return (
    <motion.div
      ...
      style={{ background: "rgba(255,123,0,0.05)", border: "1px solid rgba(255,123,0,0.18)" }}
    >
      <div className="px-4 py-3 flex items-center gap-2" style={{ borderBottom: "1px solid rgba(255,123,0,0.12)" }}>
        <span className="text-sm font-bold" style={{ color: "#ff9500" }}>📋 Thông số kỹ thuật</span>
        <span className="text-[10px] ml-auto" style={{ color: "rgba(255,255,255,0.28)" }}>
          Công thức: Móng 50% + Thân {summary.floors}×100% + Mái 50%
        </span>
      </div>
      <div className="divide-y" style={{ borderColor: "rgba(255,255,255,0.04)" }}>
        {rows.map((r) => (
          <div key={r.label} className="flex items-center justify-between px-4 py-2.5">
            <span className="text-xs" style={{ color: "rgba(255,255,255,0.42)" }}>{r.label}</span>
            <span
...
export default function Step2ChatRequirements({ basicInfo, onComplete }: Props) {
  ...
  function addBot(content: string, chips?: string[], delay = 600) {
    ...
    setTimeout(() => {
      setIsTyping(false);
      const msg = makeBotMsg(content, chips);
  ...
  useEffect(() => {
    const welcome = buildWelcomeMessage(lotWidth, lotLength, floors);
    addBot(welcome, ["Xây hết đất 🏗️", "Có chừa sân 🌿"], 500);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  function handleChip(chip: string) {
    setCurrentChips([]);
    const userMsg = makeUserMsg(chip);
  ...
  function processChip(chip: string) {
    if (stage === "ask_setback_mode") {
      ...
      const updatedFacts: ExtractedFacts = {
        ...
        setbackMode: fullBuild ? "none" : "total",
      };
    ...
    if (stage === "confirm_budget") {
      const accepted = chip.startsWith("✅");
      const updatedFacts = { ...facts, userAcceptedBudget: accepted };
      setFacts(updatedFacts);
      if (accepted || chip.includes("Ổn")) {
        startFollowup(updatedFacts);
      } else {
        addBot(
          "Không sao! Bạn có thể điều chỉnh ngân sách ở bước trước. Tuy nhiên, tôi vẫn tiếp tục phác thảo theo thông số hiện tại. Bạn có muốn tiếp tục không?",
    ...
    if (stage === "followup_requirements") {
      ...
      if (nextIndex < FOLLOWUP_QUESTIONS.length) {
        ...
        setCurrentChips([]);
        setStage("completed");
        const completionMsg = buildCompletionMessage(newFollowup);
        addBot(completionMsg, undefined, 700);
      }
    }
  }
  ...
  function handleSetbackConfirm(meters: number) {
    ...
    setInputError(null);
    const buildLen = lotLength - meters;
    if (buildLen <= 0) {
      setInputError("Chiều dài xây dựng phải lớn hơn 0.");
      return;
    }
    const updatedFacts: ExtractedFacts = { ...facts, setback: meters };
    setFacts(updatedFacts);
    startCalculating(updatedFacts, meters);
  }
  ...
  function startCalculating(updatedFacts: ExtractedFacts, setback: number) {
    ...
    setTimeout(() => {
      ...
      const calcMsg = buildCalculationMessage(newSummary);
      const msg = makeBotMsg(calcMsg, ["✅ Ổn với tôi", "💸 Hơi cao", "✏️ Tôi muốn điều chỉnh"]);
      setMessages((prev) => [...prev, msg]);
      setCurrentChips(["✅ Ổn với tôi", "💸 Hơi cao", "✏️ Tôi muốn điều chỉnh"]);
  ...
  function startFollowup(updatedFacts: ExtractedFacts) {
    const intro = buildFollowupIntroMessage();
    setStage("followup_requirements");
    setFollowupIndex(0);
    addBot(intro, undefined, 500);
    setTimeout(() => {
  ...
  function handleSendText() {
    ...
    if (stage === "ask_setback_value") {
      ...
      setInputError(null);
      setCurrentChips([]);
      const userMsg = makeUserMsg(`Chừa ${meters}m`);
      setMessages((prev) => [...prev, userMsg]);
      setInputValue("");
      scrollToBottom();
  ...
  const inputDisabled = isTyping || stage === "calculating" || stage === "ask_setback_mode" || stage === "confirm_budget" || stage === "followup_requirements" || stage === "completed";
        ...
        {stage !== "completed" && (
          ...
          >
            <input
              ...
              placeholder={
                stage === "ask_setback_value"
                  ? "Nhập số mét chừa (VD: 2 hoặc 1.5)..."
                  : "Chọn phương án bên trên..."
              }
            ...
            <motion.button
              ...
              style={{
                ...
                opacity: !inputDisabled && inputValue.trim() ? 1 : 0.3,
              }}
            >
